// tests/unit/buffer/test_shared_buffer.c
...
{
    TEST_ASSERT_EQUAL(BUFFER_ERR_NULL_INPUT,
        shared_buffer_produce(NULL, &s_data));
}

TEST(SharedBuffer, ProduceNullDataReturnsNullInput)
{
    TEST_ASSERT_EQUAL(BUFFER_ERR_NULL_INPUT,
        shared_buffer_produce(&s_buf, NULL));
}
...
TEST(SharedBuffer, ConsumeAfterProduceReturnsOk)
{
    ...
    shared_buffer_produce(&s_buf, &s_data);
    TEST_ASSERT_EQUAL(BUFFER_OK,
        shared_buffer_consume(&s_buf, &out));
}
...
static void *producer_thread(void *arg)
{
    ...
    args->result = shared_buffer_produce(args->buf, &args->data);
    return NULL;
}
...
{
    ThreadArgs *args = (ThreadArgs *)arg;
    args->result = shared_buffer_consume(args->buf, &args->data);
    return NULL;
}
